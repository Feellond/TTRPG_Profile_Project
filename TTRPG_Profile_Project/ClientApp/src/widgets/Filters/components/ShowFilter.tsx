...
import { CreatureFilterDTO, ItemFilterDTO, SpellFilterDTO } from "shared/models";
import { ItemFilter } from "./ItemFilter";
import { BestiaryFilter, SpellFilter } from "..";

type Props = {
  filter: ItemFilterDTO | CreatureFilterDTO | SpellFilterDTO;
  setFilter: React.Dispatch<React.SetStateAction<ItemFilterDTO | CreatureFilterDTO | SpellFilterDTO>>;
};
...
const ShowFilter: FC<Props> = ({ filter, setFilter }) => {
  ...
  if ("itemType" in filter) {
    ...
    content = (
      <div>
        <BestiaryFilter filter={filter as CreatureFilterDTO} setFilter={setFilter}/>
      </div>
    );
  }
